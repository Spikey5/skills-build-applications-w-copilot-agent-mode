...
function Leaderboard() {
  ...
  useEffect(() => { fetch(apiUrl('leaderboard')).then((response) => { if (!response.ok) throw new Error(); return response.json() }).then((payload) => { setRows(collectionFrom(payload)); setStatus('ready') }).catch(() => setStatus('error')) }, [])
