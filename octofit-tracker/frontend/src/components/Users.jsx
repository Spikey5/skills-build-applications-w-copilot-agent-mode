...
function Users() {
  ...
  useEffect(() => { fetch(usersUrl).then((response) => { if (!response.ok) throw new Error(); return response.json() }).then((payload) => { setUsers(collectionFrom(payload)); setStatus('ready') }).catch(() => setStatus('error')) }, [])
