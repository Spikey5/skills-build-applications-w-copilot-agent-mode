...
function Teams() {
  ...
  useEffect(() => { fetch(apiUrl('teams')).then((response) => { if (!response.ok) throw new Error(); return response.json() }).then((payload) => { setTeams(collectionFrom(payload)); setStatus('ready') }).catch(() => setStatus('error')) }, [])
